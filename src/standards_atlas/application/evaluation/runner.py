"""Execution and comparison of prompt, model, and dataset evaluations."""
...
class EvaluationRunner:
    ...
    def run(
        ...
        *,
        ...
        cases = tuple(
            self._run_case(prompt, example, model, dataset.version) for example in dataset.examples
        )
    ...
    def _run_case(self, prompt, example, model, dataset_version):
        ...
        request = StructuredGenerationRequest(
            ...
            temperature=0.0,
            seed=0,
            metadata={"example_id": example.id, "dataset_version": dataset_version},
        )
        try:
