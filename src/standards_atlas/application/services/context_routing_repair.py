...
def repair_context_routing(document: EngineeringDocument) -> ContextRoutingRepair:
    ...
    for clause in refreshed.clauses:
        ...
        if protection and (routing.references or routing.scopes):
            ...
            normalized = normalize_context_routing_targets(
                routing, refreshed, index=index, diagnostics=diagnostics
            )
    ...
    report = {
        ...
        "requires_review": sum(
            item["status"] not in {"resolved", "external", "protected"} for item in diagnostics
        ),
