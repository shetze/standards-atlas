...
class AtlasDataRoundTripWriter:
    ...
    def update_toc(
        ...
        *,
        ...
        existing_keys = {(record.kind, record.reference) for record in updated_records}

        removed_records = sum(
            (record.kind, record.reference) not in existing_keys for record in existing_records
        )

        generated_toc_records = sum(record.kind == "TOC" for record in generated_records)
    ...
    def _merge_records(
        ...
        *,
        ...
        existing_toc = {
            record.reference: record for record in existing_records if record.kind == "TOC"
        }
        ...
        public_text_count = sum(record.kind == "PublicTXT" for record in merged)
