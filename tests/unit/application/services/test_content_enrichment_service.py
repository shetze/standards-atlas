...
def test_enriches_clause_ranges_and_removes_structural_heads(tmp_path):
    ...
    assert (
        first_generated["baseline.reference_mentions"].generator == "reference-mention-extractor/v3"
    )
    ...
    return Standard(
        ...
        clauses=tuple(
            Clause(
                ...
                heading="AtlasData fallback" if number == "2" else None,
            )
            for number in ("1", "2")
        ),
    )
