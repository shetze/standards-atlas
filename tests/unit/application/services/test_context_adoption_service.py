...
def test_preview_never_writes_and_replay_never_resaves():
    ...
    service = ContextAdoptionService(documents=docs)
...
def test_document_selection_does_not_require_unselected_targets():
    ...
    result = ContextAdoptionService(documents=docs).apply(
...
def test_missing_document_is_not_created_from_archived_text():
    ...
    with pytest.raises(FileNotFoundError):
        ContextAdoptionService(documents=docs).apply(batch(candidate()), write=True)
    assert docs.saves == []
...
def test_protected_value_is_reported_and_not_confirmed_by_acceptance():
    ...
    docs = Documents(doc)
    result = ContextAdoptionService(documents=docs).apply(batch(candidate()), write=True)
...
def test_report_serialization_preserves_explicit_false_and_no_text_in_provenance():
    docs = Documents(document())
    report = ContextAdoptionService(documents=docs).apply(batch(candidate()))
