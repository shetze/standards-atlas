...
def test_qualification_plan_requires_taxonomy_and_semantic_profile_classification() -> None:
    ...
    assert any("enrich-context" in _command(step) for step in plan.steps)
...
def test_docling_is_reused_unless_regeneration_is_requested() -> None:
    ...
    assert _command(regenerated_docling)[-1] == "--overwrite"
...
def test_overwrite_propagates_to_derived_document_and_matrix_steps() -> None:
    ...
    assert "--no-fail-on-matrix-failure" in _command(matrix)
    assert _command(matrix)[-1] == "--overwrite"
    assert any(
        step.stage is WorkflowStage.NORMALIZE and "--overwrite" in _command(step)
