...
class HeadingFactory():
    ...
    def generate_headings(self, clause, verbose = False):
        ...
        while len(headings)==0:
            ...
            if attempt > 5:
                logger.warning(f"no heading offer from {self.model} for {text} in {response['response']}")
                return headings
        return headings

    def generate_alternative_headings(self, clause, cacheFile = None, force = False, verbose = False):
                ...
                for line in store:
                    if re.match('^#',line):
                        ...
                        if match:
                            ...
                            clause.heading.addAlternative(match[1], 'generated', self.model)
        except IOError as e:
            logger.warning(f"file open error: {e}")
