...
12. **Assertion proposal extraction and formal projection** writes run-scoped `DocumentKnowledgeProposal` artifacts containing source-bound entity and assertion proposals against the selected domain-specific OWL TBox/RBox vocabulary. Slice 5B grounds every proposed entity and assertion through an exact unique quote resolved to `Clause.plain_text`; absent or repeated quotes become explicit proposal violations and never fall back to whole-clause evidence. Normative force is assertion-local. Confidence, rationale, violations, attempts and failures remain proposal metadata. Only qualified/adopted assertions become canonical `DocumentKnowledge`; every accepted assertion retains source and evidence anchors.
