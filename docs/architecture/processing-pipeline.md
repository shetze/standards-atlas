...
12. **Formal semantic extraction and projection** writes run-scoped `DocumentKnowledgeProposal` artifacts containing source-bound entity and assertion proposals against the selected domain-specific OWL TBox/RBox vocabulary. Confidence, rationale, violations, attempts and failures remain proposal metadata. Only qualified/adopted assertions become canonical `DocumentKnowledge`; every accepted assertion retains source and evidence anchors.
