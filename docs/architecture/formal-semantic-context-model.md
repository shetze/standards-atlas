...
The former extraction-to-ABox augmentation path has been removed. Slice 5A defines assertion-centred `DocumentKnowledgeProposal` as the non-canonical proposal contract and persists it under a run-scoped proposal workspace. Slice 5B adds the assertion extractor and exact evidence-grounding path: successful model quotes become offset/hash `EvidenceAnchor` values only when they occur exactly once in `Clause.plain_text`; absent or ambiguous quotes are rejected as violations. Slice 5C removes the superseded entity/relation extraction contract and its qualification/archive infrastructure completely. Neither proposal persistence nor extraction confers authority or mutates canonical `EngineeringDocument.knowledge`.
