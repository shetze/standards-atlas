...
The former `SemanticExtractionProjectionAugmenter` has been removed. Slice 5A defines assertion-centred `DocumentKnowledgeProposal` as the target non-canonical proposal contract and persists it under a run-scoped proposal workspace. Slice 5B adds the new assertion extractor and exact evidence-grounding path: successful model quotes become offset/hash `EvidenceAnchor` values only when they occur exactly once in `Clause.plain_text`; absent or ambiguous quotes are rejected as violations. Transitional `DocumentSemanticExtraction` remains only for the old qualification path until Slice 5C removes that contract. Neither proposal persistence nor extraction confers authority or mutates canonical `EngineeringDocument.knowledge`.
