...
The former extraction-to-ABox augmentation path has been removed. Slice 5A defines assertion-centred `DocumentKnowledgeProposal` as the non-canonical proposal contract and persists it under a run-scoped proposal workspace. Slice 5B adds the assertion extractor and exact evidence-grounding path. `EvidenceAnchor` identifies both the source clause and canonical source surface (`body` or `heading`); successful entity quotes may resolve against the local body, local heading, or an ancestor heading supplied by the canonical CBox, while assertion evidence remains local to the source clause body. Absent or ambiguous quotes are rejected as violations. Slice 5C removes the superseded entity/relation extraction contract and its qualification/archive infrastructure completely. Neither proposal persistence nor extraction confers authority or mutates canonical `EngineeringDocument.knowledge`.
