...
The former `SemanticExtractionProjectionAugmenter` has been removed. Slice 5A now defines assertion-centred `DocumentKnowledgeProposal` as the target non-canonical proposal contract and persists it under a run-scoped proposal workspace. Transitional `DocumentSemanticExtraction` results remain proposal/qualification artifacts only and have no direct path into the ABox; the existing extractor continues to use that legacy artifact until its Slice 5B/5C cut-over. Proposal persistence itself confers no authority and cannot mutate canonical `EngineeringDocument.knowledge`.
