...
- `functional-safety/2.1.0` imports Core 2.0 and adds the Functional Safety extension, including the deterministic qualified-recommendation vocabulary used by Slice 6C.
...
Slice 4A establishes Formal Ontology 2.0 and the explicit source-extraction vocabulary boundary. The current projection uses `standards-atlas-core@2.0.0` and, when Functional Safety context is present, `functional-safety@2.1.0`. The core CBox vocabulary includes deterministic `primarySubject`, `subjectConfidence`, and `subjectEvidenceKind` facets. A Turtle adapter emits direct RDF triples plus reified `stat:SemanticAssertion` and explicit context-facet resources. The provider-neutral projection can also be persisted as versioned JSON under `.atlas/data/formal-semantic-projections/`.
...
The former extraction-to-ABox augmentation path has been removed. Slice 5A defines assertion-centred `DocumentKnowledgeProposal` as the non-canonical proposal contract and persists it under a run-scoped proposal workspace. Slice 5B adds the assertion extractor and exact evidence-grounding path. `EvidenceAnchor` identifies both the source clause and canonical source surface (`body` or `heading`); successful entity quotes may resolve against the local body, local heading, an ancestor heading, or a source surface explicitly supplied as associative structural context by the canonical CBox, while assertion evidence remains local to the source clause body. Absent or ambiguous quotes are rejected as violations. Slice 5C removes the superseded entity/relation extraction contract and its qualification/archive infrastructure completely. Neither proposal persistence nor extraction confers authority or mutates canonical `EngineeringDocument.knowledge`.
