...
Concrete adapters are selected at the executable boundary. Application services should be constructible in tests with in-memory or temporary-filesystem implementations. The application boundary deliberately separates structural taxonomy (`application.structure` plus `StructuralTaxonomyService`) from semantic ontology (`application.ontology` plus `OntologyClassificationService`). `SemanticRoutingService` sits between them as an integration service that consumes taxonomy evidence and emits routing artifacts without assigning ontology values. No application service may own both taxonomy and ontology classification responsibilities.
