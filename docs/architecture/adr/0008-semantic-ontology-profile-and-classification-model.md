...
- `EvidenceAnchor`: a text-safe reference to a canonical clause surface or character range; `source_clause_id` identifies the owning clause and `source_kind` distinguishes `body` from `heading`, while protected source text is not copied into the knowledge record;
...
Slice 5A introduces `DocumentKnowledgeProposal` schema 1 as the non-canonical assertion proposal boundary. It mirrors the semantic shape needed for later adoption (`EvidenceAnchor`, entity proposals, assertion proposals and ontology bindings) while retaining proposal-only confidence, rationale, attempts, failures, violations and model provenance. Proposals are persisted under run-scoped `.atlas/data/knowledge-proposals/<run-id>/` paths and have no implicit conversion into `EngineeringDocument.knowledge`. Slice 5B adds the assertion-centred extractor. The generalized grounding contract treats clause headings and deterministic associative structural context as first-class canonical evidence surfaces for entity identification while preserving exact source provenance: entity evidence may be grounded in the local body, local heading, an ancestor heading, or a body/heading surface explicitly transported by assertion CBox 1.3. Heading-only parents may contribute only their own sibling group's first earlier substantive descendant, while higher empty ancestors do not import unrelated branches. Misdeclared entity evidence sources may be recovered only for a unique exact match across allowed surfaces; assertion evidence remains local to the clause body. Missing or repeated quotes remain proposal violations, and accepted applicability state is context rather than an extraction-eligibility prerequisite. Slice 5C removes the former entity/relation extraction contract, repository, qualification command and schema family completely; there is no compatibility reader or workflow stage for those obsolete artifacts.
