...
workflow envelope before execution is introduced.
...
intentionally preserves current semantic behaviour. Taxonomy-aware task selection and workflow
execution belong to later slices.
...
The workflow manifest registry can now discover one routing-contract manifest alongside the
standards and qualification-matrix manifests, but merely supplying it does not yet execute
routing. That execution boundary is introduced separately so persistence/versioning can be
qualified without changing LLM behaviour.
