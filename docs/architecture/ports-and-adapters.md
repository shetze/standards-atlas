...
Slice 5A adds `DocumentKnowledgeProposalRepository` as a separate outbound port for rebuildable, non-canonical assertion proposals. Its filesystem adapter stores each proposal below a run-scoped `.atlas/data/knowledge-proposals/<run-id>/` directory. Slice 5B adds the provider-neutral `KnowledgeProposalExtractor` port and an application service that aggregates source-grounded per-clause results into that run artifact. The LLM adapter may propose terms only from the explicit source-extraction ontology view; deterministic grounding and run aggregation remain application concerns. Neither port exposes adoption or canonical-document mutation; qualification and adoption remain later application boundaries.
